...
with lib; let
  cfg = config.custom.arion.ghost;
in {
  options.custom.arion.ghost = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.ghost.settings.services = {
      ...
      in {
        ...
        volumes = ["${config.custom.arion.directory}/ghost/data:/var/lib/ghost/content"];
      ...
      db.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/ghost/db:/var/lib/mysql"];
      };
    };
  };
}
