...
with lib; let
  cfg = config.custom.arion.caddy;
in {
  options.custom.arion.caddy = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.caddy.settings.services = {
      ...
      caddy.service = {
        ...
        volumes = [
          "${config.custom.arion.directory}/caddy/config:/config"
          "${config.custom.arion.directory}/caddy/data:/data"
          "${config.custom.arion.directory}/caddy/etc:/etc/caddy:ro"
          "/srv:/srv:ro"
        ];
      };
    };
    ...
    systemd = {
      ...
      in {
        "${config.custom.arion.directory}/caddy/config" = {
          d = owner "0700"; # -rwx------
          z = owner "0700"; # -rwx------
        };

        "${config.custom.arion.directory}/caddy/data" = {
          d = owner "0700"; # -rwx------
          z = owner "0700"; # -rwx------
        };
        ...
        "${config.custom.arion.directory}/caddy/etc" = {
