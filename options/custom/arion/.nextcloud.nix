...
with lib; let
  cfg = config.custom.arion.nextcloud;
in {
  options.custom.arion.nextcloud.enable = mkOption {default = false;};
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.nextcloud.settings.services = {
      ...
      nextcloud.service = {
        ...
        volumes = [
          "${config.custom.arion.directory}/nextcloud/app:/var/www/html"
          "${config.custom.arion.directory}/nextcloud/data:/var/www/html/data"
        ];
      };
      ...
      cache.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/nextcloud/cache:/data"];
      };
      ...
      db.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/nextcloud/db:/var/lib/postgresql/data"];
      };
    };
  };
}
