...
with lib; let
  cfg = config.custom.arion.opengist;
in {
  options.custom.arion.opengist = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.opengist.settings.services = {
      ...
      opengist.service = {
        ...
        volumes = ["${config.custom.arion.directory}/opengist/data:/opengist"];
      ...
      vpn.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/opengist/vpn:/var/lib/tailscale"];
    ...
    in {
      "${config.custom.arion.directory}/opengist/data" = {
        d = owner "0700"; # -rwx------
        z = owner "0700"; # -rwx------
      };
    };
