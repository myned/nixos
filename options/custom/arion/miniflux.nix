...
with lib; let
  cfg = config.custom.arion.miniflux;
in {
  options.custom.arion.miniflux = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.miniflux.settings.services = {
      ...
      miniflux.service = {
        ...
        volumes = ["${config.custom.arion.directory}/miniflux/data:/data"];
      };
      ...
      db.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/miniflux/db:/var/lib/postgresql/data"];
      };
    };
  };
}
