...
with lib; let
  cfg = config.custom.arion.synapse;
in {
  options.custom.arion.synapse = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.synapse.settings.services = {
      ...
      synapse.service = {
        ...
        volumes =
          [
            "${config.custom.arion.directory}/synapse/data:/data"
      ...
      db.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/synapse/db:/var/lib/postgresql/data"];
      };
      ...
      vpn.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/synapse/vpn:/var/lib/tailscale"];
    ...
    in
      {
        "${config.custom.arion.directory}/synapse/data" = {
          d = owner "0700";
          z = owner "0700";
        };
      }
