...
with lib; let
  cfg = config.custom.arion.openwebui;
in {
  options.custom.arion.openwebui = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.openwebui.settings.services = {
      ...
      openwebui.service = {
        ...
        volumes = ["${config.custom.arion.directory}/openwebui/data:/app/backend/data"];
      ...
      vpn.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/openwebui/vpn:/var/lib/tailscale"];
