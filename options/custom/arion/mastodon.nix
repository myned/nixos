...
with lib; let
  cfg = config.custom.arion.mastodon;
in {
  options.custom.arion.mastodon = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.mastodon.settings.services = {
      ...
      mastodon.service = {
        ...
        volumes = ["${config.custom.arion.directory}/mastodon/config:/config"];
      };
      ...
      cache.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/mastodon/cache:/data"];
      };
      ...
      db.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/mastodon/db:/var/lib/postgresql/data"];
      };
    };
