...
with lib; let
  cfg = config.custom.arion.jellyfin;
in {
  options.custom.arion.jellyfin = {
    enable = mkEnableOption "jellyfin";

    dataDir = mkOption {
      default = "${config.custom.arion.directory}/jellyfin/data";
      type = types.str;
    };
  ...
  config = mkIf cfg.enable {
      ...
      jellyfin.settings.services =
        {
          ...
          jellyfin.service = {
            ...
            volumes = [
              "${config.custom.arion.directory}/jellyfin/cache:/cache"
              "${config.custom.arion.directory}/jellyfin/config:/config"
              "${cfg.dataDir}:/data"
            ];
          };
          ...
          jellyseerr.service = {
            ...
            restart = "unless-stopped";
            user = "${cfg.uid}:${cfg.gid}";
            volumes = ["${config.custom.arion.directory}/jellyfin/jellyseerr:/app/config"];
          };
          ...
          lidarr.service = {
            ...
            volumes = [
              "${config.custom.arion.directory}/jellyfin/lidarr:/config"
              "${cfg.dataDir}:/data"
            ];
          ...
          prowlarr.service = {
            ...
            restart = "unless-stopped";
            volumes = ["${config.custom.arion.directory}/jellyfin/prowlarr:/config"];
          ...
          qbittorrent.service = {
            ...
            volumes = [
              "${config.custom.arion.directory}/jellyfin/qbittorrent:/config"
              "${cfg.dataDir}:/data"
            ];
          ...
          radarr.service = {
            ...
            volumes = [
              "${config.custom.arion.directory}/jellyfin/radarr:/config"
              "${cfg.dataDir}:/data"
            ];
          ...
          slskd.service = {
            ...
            volumes = [
              "${config.custom.arion.directory}/jellyfin/slskd:/app"
              "${cfg.dataDir}:/data"
            ];
          ...
          sonarr.service = {
            ...
            volumes = [
              "${config.custom.arion.directory}/jellyfin/sonarr:/config"
              "${cfg.dataDir}:/data"
            ];
          ...
          soularr.service = {
            ...
            volumes = [
              "${config.custom.arion.directory}/jellyfin/soularr:/data"
          ...
          vpn.service = {
            ...
            restart = "unless-stopped";
            volumes = ["${config.custom.arion.directory}/jellyfin/vpn:/var/lib/tailscale"];
        ...
        // optionalAttrs cfg.inotify {
          ...
          inotify.service = {
            ...
            volumes = let
              ...
              '';
            in [
              "${config.custom.arion.directory}/jellyfin/soularr:/data:ro"
              "${notify}:/notify.sh:ro"
            ];
    ...
    in
      listToAttrs (map (name: {
          ...
          "${cfg.dataDir}/music"
          "${cfg.dataDir}/shows"
          "${config.custom.arion.directory}/jellyfin/blackhole"
          "${config.custom.arion.directory}/jellyfin/cache"
          "${config.custom.arion.directory}/jellyfin/config"
          "${config.custom.arion.directory}/jellyfin/jellyseerr"
          "${config.custom.arion.directory}/jellyfin/lidarr"
          "${config.custom.arion.directory}/jellyfin/prowlarr"
          "${config.custom.arion.directory}/jellyfin/qbittorrent"
          "${config.custom.arion.directory}/jellyfin/radarr"
          "${config.custom.arion.directory}/jellyfin/slskd"
          "${config.custom.arion.directory}/jellyfin/sonarr"
        ]);
