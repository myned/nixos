...
with lib; let
  cfg = config.custom.arion.beszel;
in {
  options.custom.arion.beszel = {
    ...
    virtualisation.arion.projects.beszel.settings.services =
      optionalAttrs cfg.server {
        beszel.service = {
          ...
          volumes = ["${config.custom.arion.directory}/beszel/data:/beszel_data"];
        };
        ...
        vpn.service = {
          ...
          restart = "unless-stopped";
          volumes = ["${config.custom.arion.directory}/beszel/vpn:/var/lib/tailscale"];
