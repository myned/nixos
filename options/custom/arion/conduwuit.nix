...
with lib; let
  cfg = config.custom.arion.conduwuit;
in {
  options.custom.arion.conduwuit.enable = mkOption {default = false;};
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.conduwuit.settings.services = {
      ...
      conduwuit.service = {
        ...
        volumes = [
          "${config.custom.arion.directory}/conduwuit/db:/var/lib/conduwuit"
