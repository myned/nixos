...
with lib; let
  cfg = config.custom.arion.directus;
in {
  options.custom.arion.directus = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.directus.settings.services = {
      ...
      directus.service = {
        ...
        volumes = [
          "${config.custom.arion.directory}/directus/db:/directus/database"
          "${config.custom.arion.directory}/directus/uploads:/directus/uploads"
          "${config.custom.arion.directory}/directus/extensions:/directus/extensions"
        ];
      };
    };
    ...
    in {
      "${config.custom.arion.directory}/directus/db" = {
        d = owner "0700"; # -rwx------
        z = owner "0700"; # -rwx------
      };

      "${config.custom.arion.directory}/directus/uploads" = {
        d = owner "0700"; # -rwx------
        z = owner "0700"; # -rwx------
      };

      "${config.custom.arion.directory}/directus/extensions" = {
        d = owner "0700"; # -rwx------
        z = owner "0700"; # -rwx------
      };
    };
