...
with lib; let
  cfg = config.custom.arion.mullvad;
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.mullvad.settings.services = {
      mullvad.service =
        ...
        // optionalAttrs (!cfg.gluetun) {
          ...
          restart = "unless-stopped";
          volumes = ["${config.custom.arion.directory}/mullvad/config:/config"];
    ...
    systemd = optionalAttrs (!cfg.gluetun) {
      services.arion-mullvad-randomize = {
        ...
        script = ''
          ${mkdir} -p ${config.custom.arion.directory}/mullvad/config/wg_confs
          cd ${config.custom.arion.directory}/mullvad/config
