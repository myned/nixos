...
with lib; let
  cfg = config.custom.arion.kener;
in {
  options.custom.arion.kener = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.kener.settings.services = {
      ...
      kener.service = {
        ...
        volumes = ["${config.custom.arion.directory}/kener/data:/app/uploads"];
      };
      ...
      db.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/kener/db:/var/lib/postgresql/data"];
      };
      ...
      vpn.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/kener/vpn:/var/lib/tailscale"];
    ...
    in {
      "${config.custom.arion.directory}/kener/data" = {
        d = owner "0700"; # -rwx------
        z = owner "0700"; # -rwx------
      };
    };
    ...
    age.secrets = listToAttrs (map (name: {
      ...
      [
        "${config.custom.hostname}/kener/.env"
        "${config.custom.hostname}/kener/db.env"
      ]);
  };
}
