...
with lib; let
  cfg = config.custom.arion.passbolt;
in {
  options.custom.arion.passbolt = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.passbolt.settings.services = {
      ...
      passbolt.service = {
        ...
        volumes = [
          "${config.custom.arion.directory}/passbolt/gpg:/etc/passbolt/gpg"
          "${config.custom.arion.directory}/passbolt/jwt:/etc/passbolt/jwt"
        ];
      };
      ...
      db.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/passbolt/db:/var/lib/postgresql/data"];
      };
    };
    ...
    in {
      "${config.custom.arion.directory}/passbolt/gpg" = {
        d = owner "0700"; # -rwx------
        z = owner "0700"; # -rwx------
      };

      "${config.custom.arion.directory}/passbolt/jwt" = {
        d = owner "0700"; # -rwx------
        z = owner "0700"; # -rwx------
      };
    };
