...
with lib; let
  cfg = config.custom.arion.statping;
in {
  options.custom.arion.statping = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.statping.settings.services = {
      ...
      statping.service = {
        ...
        volumes = ["${config.custom.arion.directory}/statping/data:/app"];
      };
      ...
      vpn.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/statping/vpn:/var/lib/tailscale"];
