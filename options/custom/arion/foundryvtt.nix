...
with lib; let
  cfg = config.custom.arion.foundryvtt;
in {
  options.custom.arion.foundryvtt.enable = mkOption {default = false;};
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.foundryvtt.settings.services = {
      foundryvtt.service = {
        ...
        volumes = ["${config.custom.arion.directory}/foundryvtt/data:/data"];
      };
    };
  };
}
