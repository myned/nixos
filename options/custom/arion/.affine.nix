...
with lib; let
  cfg = config.custom.arion.affine;
in {
  options.custom.arion.affine = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.affine.settings.services = {
      ...
      affine.service = {
        ...
        volumes = ["${config.custom.arion.directory}/affine/data:/root/.affine"];
      ...
      db.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/affine/db:/var/lib/postgresql/data"];
