...
with lib; let
  cfg = config.custom.arion.syncthing;
in {
  options.custom.arion.syncthing = {
    ...
    virtualisation.arion.projects.syncthing.settings.services =
      ...
      optionalAttrs (!cfg.server) {
        syncthing.service = {
          ...
          volumes = ["${config.custom.arion.directory}/syncthing/data:/data"];
        };
      }
      ...
      // {
        ...
        vpn.service = {
          ...
          restart = "unless-stopped";
          volumes = ["${config.custom.arion.directory}/syncthing/vpn:/var/lib/tailscale"];
