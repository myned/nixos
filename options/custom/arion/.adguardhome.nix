...
with lib; let
  cfg = config.custom.arion.adguardhome;
in {
  options.custom.arion.adguardhome = {
    ...
    virtualisation.arion.projects.adguardhome.settings.services = {
      ...
      adguardhome.service = {
        ...
        volumes = [
          ...
          "${config.custom.arion.directory}/adguardhome/config:/opt/adguardhome/conf"
          "${config.custom.arion.directory}/adguardhome/data:/opt/adguardhome/data"
        ];
      };
      ...
      vpn.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/adguardhome/vpn:/var/lib/tailscale"];
