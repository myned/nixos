...
with lib; let
  cfg = config.custom.arion.netbox;
in {
  options.custom.arion.netbox.enable = mkOption {default = false;};
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.netbox.settings.services = let
      netbox = {
        ...
        user = "unit:root";
        volumes = ["${config.custom.arion.directory}/netbox/media:/opt/netbox/netbox/media"];
      };
    in {
      netbox.service =
        netbox
      ...
      db.service = {
        ...
        image = "postgres:16";
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/netbox/db:/var/lib/postgresql/data"];
      };
    };

    #!! Required for correct volume permissions
    systemd.tmpfiles.settings.netbox = {
      "${config.custom.arion.directory}/netbox/media" = {
        z = {
          mode = "0770";
          user = "999"; # unit
          group = "root";
