...
with lib; let
  cfg = config.custom.arion.rconfig;
in {
  options.custom.arion.rconfig = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.rconfig.settings.services = {
      ...
      rconfig.service = {
        ...
        volumes = ["${config.custom.arion.directory}/rconfig/data:/var/www/html/rconfig/storage"];
      };
      ...
      db.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/rconfig/db:/var/lib/mysql"];
      };
      ...
      vpn.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/rconfig/vpn:/var/lib/tailscale"];
