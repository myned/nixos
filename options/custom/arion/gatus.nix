...
with lib; let
  cfg = config.custom.arion.gatus;
in {
  options.custom.arion.gatus = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.gatus.settings.services = {
      ...
      gatus.service = {
        ...
        volumes =
          [
            "${config.custom.arion.directory}/gatus/data:/data"
          ]
      ...
      vpn.service = {
        ...
        ports = ["8484:8080/tcp"];
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/gatus/vpn:/var/lib/tailscale"];
