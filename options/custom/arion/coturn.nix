...
with lib; let
  cfg = config.custom.arion.coturn;
in {
  options.custom.arion.coturn.enable = mkOption {default = false;};
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.coturn.settings.services = {
      ...
      coturn.service = {
        ...
        image = "coturn/coturn:4.7.0"; # https://hub.docker.com/r/coturn/coturn/tags
        network_mode = "host";
        restart = "unless-stopped";

        volumes = [
          "${config.custom.arion.directory}/coturn/coturn.conf:/etc/coturn/turnserver.conf"
        ];
      };
    };
    ...
    systemd.tmpfiles.settings.coturn = {
      "${config.custom.arion.directory}/coturn/coturn.conf" = {
        C = {
          mode = "0444";
