...
with lib; let
  cfg = config.custom.arion.portainer;
in {
  options.custom.arion.portainer = {
    ...
    virtualisation.arion.projects.portainer.settings.services =
      ...
      optionalAttrs cfg.server {
        portainer.service = {
          ...
          volumes = ["${config.custom.arion.directory}/portainer/data:/data"];
        };
      }
      ...
      // optionalAttrs cfg.agent {
        agent.service = {
          ...
          volumes = [
            "${config.custom.arion.directory}:/host:ro"
      ...
      // {
        vpn.service = {
          ...
          restart = "unless-stopped";
          volumes = ["${config.custom.arion.directory}/portainer/vpn:/var/lib/tailscale"];
