...
with lib; let
  cfg = config.custom.arion.uptimekuma;
in {
  options.custom.arion.uptimekuma = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.uptimekuma.settings.services = {
      ...
      uptimekuma.service = {
        ...
        volumes = ["${config.custom.arion.directory}/uptimekuma/data:/app/data"];
      };
      ...
      vpn.service = {
        ...
        restart = "unless-stopped";
        volumes = ["${config.custom.arion.directory}/uptimekuma/vpn:/var/lib/tailscale"];
