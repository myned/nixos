...
with lib; let
  cfg = config.custom.arion.forgejo;
in {
  options.custom.arion.forgejo = {
  ...
  config = mkIf cfg.enable {
    ...
    virtualisation.arion.projects.forgejo.settings.services =
      optionalAttrs cfg.server {
        ...
        forgejo.service = {
          ...
          volumes = ["${config.custom.arion.directory}/forgejo/data:/var/lib/gitea"];
        };
        ...
        db.service = {
          ...
          restart = "unless-stopped";
          volumes = ["${config.custom.arion.directory}/forgejo/db:/var/lib/postgresql/data"];
        };
        ...
        vpn.service = {
          ...
          restart = "unless-stopped";
          volumes = ["${config.custom.arion.directory}/forgejo/vpn:/var/lib/tailscale"];
      ...
      // optionalAttrs cfg.runner {
        ...
        runner.service = {
          ...
          restart = "unless-stopped";
          user = "1001:1001";
          volumes = ["${config.custom.arion.directory}/forgejo/runner:/data"];
    ...
    systemd.tmpfiles.settings = {
      ...
      in
        mkIf cfg.server {
          "${config.custom.arion.directory}/forgejo/data" = {
            d = owner "0700"; # -rwx------
            z = owner "0700"; # -rwx------
          };
        };
      ...
      in
        mkIf cfg.runner {
          "${config.custom.arion.directory}/forgejo/runner" = {
            d = owner "0700"; # -rwx------
            z = owner "0700"; # -rwx------
          };
        };
