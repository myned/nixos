...
with lib; let
  cfg = config.custom.arion.grafana;
in {
  options.custom.arion.grafana = {
      ...
      services =
        {
          ...
          grafana.service = {
            ...
            volumes = ["${config.custom.arion.directory}/grafana/data:/var/lib/grafana"];
          ...
          vpn.service = {
            ...
            restart = "unless-stopped";
            volumes = ["${config.custom.arion.directory}/grafana/vpn:/var/lib/tailscale"];
        ...
        // optionalAttrs cfg.prometheus.enable {
          ...
          prometheus.service = {
            ...
            volumes = [
              ...
              "${config.custom.arion.directory}/grafana/prometheus/data:/prometheus"
            ];
          };
        };
    };
    ...
    systemd.tmpfiles.settings =
      {
        ...
        in {
          "${config.custom.arion.directory}/grafana/data" = {
            d = owner "0700"; # -rwx------
            z = owner "0700"; # -rwx------
          };
        };
      ...
      // optionalAttrs cfg.prometheus.enable {
        ...
        in {
          "${config.custom.arion.directory}/grafana/prometheus/data" = {
            d = owner "0700"; # -rwx------
            z = owner "0700"; # -rwx------
          };
        };
