...
in {
  ...
  config = mkIf cfg.enable {
    ...
    environment.systemPackages = [hm.programs.starship.package];
    ...
    home-manager.sharedModules = [
      {
        programs = {
          starship = {
            ...
            settings = (foldr (a: b: recursiveUpdate a b) {}) (
              ### Presets
              # https://starship.rs/presets/
              #?? starship preset <preset> | wl-copy
              #!! Manually import from presets directory
              # https://github.com/nix-community/home-manager/issues/4231
              (map (file: fromTOML (readFile file)) (filesystem.listFilesRecursive ./presets))
              ++ [
                {
                  ### Overrides
