...
in {
  ...
  config = mkIf cfg.enable {
    ...
    systemd.user.tmpfiles.rules = [
      "L+ ${config.custom.games.steamDir}/617290/pfx/drive_c/users/steamuser/AppData/Local/Remnant/Saved/SaveGames - - - - ${config.custom.sync}/game/saves/remnant/${config.custom.username}/SaveGames"
    ];
  };
}
